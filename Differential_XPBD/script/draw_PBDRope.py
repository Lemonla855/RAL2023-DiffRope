...
class ProcessOptData():
    ...
    def SaveData(self, save_loss_itr, save_curr_itr_time, save_total_itr_time,
                 save_nodes_pos_itr, save_nodes_radius_itr, node_num, save_dir,
                 num_frame):
        ### ==========================================================================================
        ### SAVE DATA
    ...
    #-------- Save the temp data for each layers and each steps --------
    def SaveTempData(self, node_pos_curr, node_radius_curr, loss, C_dis,
                     C_strain, C_bending, save_dir, num_frame, itr, i, j):
        #pdb.set_trace()

        # -------- Just save the constraints with maximum value --------
        save_optimized_temp_data = np.hstack((
            node_pos_curr,
            node_radius_curr.reshape(-1, 1),
            np.insert(C_dis, 19, [loss]).reshape(-1, 1),
            np.insert(torch.max(C_strain, dim=1).values, 19,
                      [0]).reshape(-1, 1),
            np.insert(torch.max(C_bending, dim=1).values, 18,
                      [0, 0]).reshape(-1, 1),
        ))

        np.save(
            save_dir + "/temp_data_frame_layer_step_frame" + str(num_frame) +
            "/frame_" + str(num_frame) + "_itr_" + str(itr) + "_step_" +
            str(j) + "_layer_" + str(i) + ".npy", save_optimized_temp_data)
...
class ExecuteBackward():
    def Execute(self, start_frame, end_frame, save_dir, LR_RATE, LAYERS, STEPS,
        ...
        for num_frame in range(BaxterRope.start_frame, BaxterRope.end_frame):
            ...
            for itr in range(BaxterRope.ITERATION):
                ...
                for j in range(BaxterRope.STEPS):
                    ...
                    for i in range(BaxterRope.LAYERS):
                        ...
                        # -------- Calculate temp loss --------
                        DiffLoss = GetLoss()
                        loss = DiffLoss.CalculateLoss(
                            cal_loss_method, cal_loss_target, node_pos_curr,
                            num_frame, BaxterRope.cam_intrinsic_mat)
                        ### -------- Save Data ---------
                        OptimizedData.SaveTempData(node_pos_curr,
                                                   node_radius_curr, loss,
                                                   C_dis, C_strain, C_bending,
                                                   BaxterRope.save_dir,
                                                   num_frame, itr, i, j)
            ...
            #                        save_nodes_radius_itr, BaxterRope.node_num,
            #                        BaxterRope.save_dir, num_frame)

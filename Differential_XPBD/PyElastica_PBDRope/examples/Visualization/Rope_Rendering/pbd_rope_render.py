...
# Setup (USER DEFINE)
DATA_PATH = "/home/jingpei/entong_ws/diff_viper_PBD/important_data/3D_traj_mul/traj_data/"  # Path to the simulation data
SAVE_PICKLE = True
...
if __name__ == "__main__":
    ...
    for i in range(TOTAL_FRAMES):
        file_name = DATA_PATH + str(i + 1) + '.npy'
        try:
            with open(file_name, "rb") as fptr:
                data = np.load(fptr)
                # pdb.set_trace()

                data = np.moveaxis(data, -1, 1)  # shape: (2, 4, num_element) --- only shape[1, :, :] is useful

                rot_mtx = np.array([[1.0000000, 0.0000000, 0.0000000], [0.0000000, 0.0000000, -1.0000000],
                                    [0.0000000, 1.0000000, 0.0000000]])

                data_rot = np.matmul(rot_mtx, data[3,0:3, :])
               

                data_radius = data[3,3, :]  # (TODO) radius could change
    ...
    for view_name in stage_scripts.keys():
        ...
        imageset = [os.path.join(imageset_path, path) for path in os.listdir(imageset_path) if path[-3:] == "png"]
        imageset.sort()

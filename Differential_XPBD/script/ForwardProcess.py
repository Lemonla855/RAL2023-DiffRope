...
class ProcessOptData():
    ...
    # -------- Save temp data --------
    def SaveTempData(self, loss_temp, C_dis, C_strain, C_bending, itr, j,
                     save_path):
        ...
        np.save(save_path + "/itr_" + str(itr) + "_steps_" + str(j) + ".npy",
                Save_Temp_Data)
    ...
    def Execute(self, start_frame, end_frame, save_dir, LR_RATE, LAYERS, STEPS,
        ...
        for num_frame in range(BaxterRope.start_frame, BaxterRope.end_frame):
            ...
            for itr in range(BaxterRope.ITERATION):
                ...
                ### --- Initialize velocity ----
                velocity = torch.zeros((BaxterRope.node_num, 3))
                ...
                for j in range(BaxterRope.STEPS):
                    if dynamics == True:
                        ...
                        node_pos_curr = node_pos_curr + (dist_velocity) / 1
                    ...
                    OptimizedData.SaveTempData(loss_temp.detach().numpy(),
                                               C_dis.detach().numpy(),
                                               C_strain.detach().numpy(),
                                               C_bending.detach().numpy(), itr,
                                               j, BaxterRope.save_dir)

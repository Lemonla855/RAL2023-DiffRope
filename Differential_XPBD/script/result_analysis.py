...
class LossAnalysis():
    ...
    def PlotViolin(self):
        ...
        plt.xlabel("Loss Type", fontsize=18)
        plt.ylabel("Loss", fontsize=18)
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)

        #plt.title("Error for 3D projection Loss", fontsize=18)
    ...
    def Plot2DViolin(self):
        ...
        plt.yticks(fontsize=14)
        plt.legend(fontsize=15)
        plt.title("Error for 2D projection Loss", fontsize=18)
...
class CompareOverXPBDAndTomas():
    ...
    def CompareTogether(self, frames, dist, bending, strain):
        ...
        files_path = [
            ...
            "../DATA/save_loss_withConstConvergency/XPBD/projection/centerline_3d_gravity/"
        ]
        ...
        # # -------- Set the title for the plot -------
        # if dist == True:
        #     plt.title("C_dist for Tomas VS GS XPBD", fontsize=18)
        # if bending == True:
        #     plt.title("C_bending for Tomas VS GS XPBD ", fontsize=18)
        # if strain == True:
        #     plt.title("C_strain for Tomas VS GS XPBD ", fontsize=18)
        ...
        for index, node_file in enumerate(files_path):
            for num_frame in frames:
                ...
                if dist == True:
                    #pdb.set_trace()
                    plt.scatter(
                        np.arange(19).astype(dtype=np.str), C_dist[:-1])
                    plt.plot(np.arange(19).astype(dtype=np.str),
                             C_dist[:-1],
                             label="Frame " + str(num_frame) + " for " +
                             labels[index])
        ...
        # ------- Set the size of relative info -------
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
        plt.xlabel("Node", fontsize=18)
        plt.ylabel(r"$C_{dist}$", fontsize=18)
        plt.ylim(0, 0.045)
    ...
    def CompareLength(self):
        ...
        for num_frame in range(30, 66):
            ...
            XPBD_sim_nodes = torch.as_tensor(
                np.load(
                    "../DATA/save_loss_withConstConvergency/XPBD/projection/centerline_3d_gravity/frame_%d.npy"
                    % num_frame))[:, :3]
        ...
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
        plt.savefig("./DATA/Length_compare.png", bbox_inches='tight')

        plt.cla()
        XPBD_Deviation = np.sum(
            abs(centerline_lengths -
                XPBD_lengths)) / centerline_lengths.shape[0]
        XPBD_Deviation_Percentance = XPBD_Deviation / centerline_lengths[0]
        print("XPBD_Deviation_Percentance", XPBD_Deviation_Percentance)
        Tomas_Deviation = np.sum(
            abs(centerline_lengths -
                Tomas_lengths)) / centerline_lengths.shape[0]
        Tomas_Deviation_Percentance = Tomas_Deviation / centerline_lengths[0]
        print("Tomas_Deviation_Percentance", Tomas_Deviation_Percentance)
        #pdb.set_trace()
        ...
        plt.savefig("./DATA/length_loss.png", bbox_inches='tight')
        plt.cla()
        ...
        plt.plot(frame,
                 ...
                 label="Expected Control Point Loss",
                 linestyle="--")

        plt.ylabel("Control Point loss", fontsize=18)
        plt.xlabel("Frame", fontsize=18)
        plt.legend(loc="upper left", fontsize=16)
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
        ...
        #plt.show()
        plt.savefig("./DATA/traj_loss.png", dpi=200, bbox_inches='tight')
        plt.cla()
...
class ForwardPlot():
    def PlotForwardXPBD(self):
        ...
        for index, frame_num in enumerate([30, 54]):
            ...
            #-------- plot distance constraint information for Tomas and XPBD --------
            # #plt.subplot(2, 1, 2)
        ...
        #-------- Ground Truth C-dist --------
        # #plt.subplot(2, 1, 1)
        # #plt.title("Distance Constraint Satification", fontsize=20)
        # plt.xlabel("Iteration", fontsize=18)
        # plt.ylabel(r"$C_{dist}$", fontsize=18)
        # plt.xticks(fontsize=14)
        # plt.yticks(fontsize=14)

        # length = np.ones((300)) * 0
        # plt.plot(np.arange(0, 10, 10 / 300),
        #          length,
...
class VisualizeRod():
    ...
    def PlotDifferenLossMatplotlib(self):
        ...
        # cal_tragets = ["centerline_3d_gravity"]
        ...
        def short_proj():
            return np.dot(Axes3D.get_proj(ax_curve), scale)

        #plt.style.use('seaborn')

        fig = plt.figure()
        ax_curve = Axes3D(fig)
        #ax_curve = fig.add_subplot(221, projection='3d')

        #ax_curve.get_proj = short_proj

        ax_curve.get_proj = lambda: np.dot(Axes3D.get_proj(ax_curve),
                                           np.diag([2.5, 1, 1, 1]))
        ...
        for cal_method in cal_methods:
            for cal_traget in cal_tragets:
                ...
                pcl = np.load(
                    "../DATA/DATA_BaxterRope/centerline_3d_gravity_clean/%d.npy"
                    % num_frame)
                sim_nodes = np.load("../DATA/save_loss_withConstConvergency/" +
                                    cal_method + "/" + cal_traget +
                                    "/frame_%d.npy" % num_frame)[:, :3]
                ...
                ax_curve.view_init(elev=-112., azim=-108)
                #ax_curve.view_init(elev=-61., azim=-90)
                ax_curve.set_xlabel("X")
                ax_curve.set_ylabel("Y")
                ax_curve.set_zlabel("Z")
                #ax_curve.set_title("Atom velocity distribution")

                plt.locator_params(axis="z", nbins=4)
                plt.locator_params(axis="x", nbins=4)
                plt.locator_params(axis="y", nbins=4)

                #ax_curve.set_facecolor('r')
                # plt.show()
                ax_curve.dist = 16
                plt.tight_layout()
                #ax_curve.set_title("OBJ" + str(index), loc="right")
                # ax_curve.text(1,
                #               1,
                #               1,
                #               "OBJ" + str(index),
                ...
                # plt.show()
                # pdb.set_trace()
                #plt.title("OBJ" + str(index), loc="left")
                #plt.legend()

                plt.savefig("./DATA/PlotDifferenLossMatplotlib/" + cal_method +
                            "_" + cal_traget + ".png",
                            dpi=300,
                            bbox_inches='tight')
    ...
    def ConnectPlotDifferenLossMatplotlib(self):
        ...
        index = 0
        RGB_IMAGE = cv.imread("../DATA/DATA_BaxterRope/rgb_raw_Baxter/40.png")
        RGB_IMAGE = cv.resize(RGB_IMAGE, (600, 550),
        ...
        Images = np.zeros((row_size * 3, col_size * 3, 3))
        for cal_method in cal_methods:
            for cal_traget in cal_tragets:
                images = cv.imread("./DATA/PlotDifferenLossMatplotlib/" +
                                   cal_method + "_" + cal_traget +
                                   ".png")[200:1400, :1884]
                font = cv.FONT_HERSHEY_SIMPLEX

                cv.putText(images, 'OBJ' + str(index + 1), (800, 70), font, 2,
                           (0, 0, 0), 2, cv.LINE_AA)

                images = cv.cvtColor(images, cv.COLOR_BGR2RGB) * 255
                # plt.imshow(images)
                # plt.show()

                #pdb.set_trace()

                # images = plt.imread("./DATA/PlotDifferenLossMatplotlib/" +
                #                     cal_method + "_" + cal_traget + ".png")
                print(cal_method + "_" + cal_traget)
                print(images.shape)
                #pdb.set_trace()
                ...
                # Images[0:550, 600 * (y_index - 1):600 *
                #        (y_index), :3] = RGB_IMAGE / 255
                # pdb.set_trace()
                #pdb.set_trace()
                images[:20, :, :3] = [1, 1, 1]

                Images[x_index * row_size:(x_index + 1) * row_size,
                       y_index * col_size:(y_index + 1) * col_size] = images
                index += 1
                #pdb.set_trace()

        plt.imshow((Images * 255).astype(np.uint8))
        plt.axis("off")
        plt.tight_layout()
        plt.savefig("./DATA/PlotDifferenLossMatplotlib" + ".png",
                    dpi=300,
                    bbox_inches='tight')
        plt.show()
        pdb.set_trace()
    ...
    def MergeImage(self):
        ...
        for cal_method in cal_methods:
            for cal_traget in cal_tragets:
                ...
                plt.axis("off")
                #plt.show()
                plt.savefig("../DATA/MergeImage/" + cal_method + "_" +
                            cal_traget + ".png")
                plt.cla()
                #pdb.set_trace()
    ...
    def ConnectImage(self):
        ...
        IMAGES = np.ones((row_size * 4, col_size * num, 3)) * 255
        frams = [30, 52, 60]

        # -------- Plot RGB Image ------
        index = 0
        for num_frame in frams:
            image = cv.imread("../DATA/DATA_BaxterRope/rgb_raw_Baxter/%d.png" %
                              int(num_frame))[200:200 + row_size,
                                              100:col_size + 100]
            #pdb.set_trace()
            image[:, col_size - 50:] = 1 * 255
            image[1050:, :] = 1 * 255
            IMAGES[:row_size,
                   index * col_size:(index + 1) * col_size] = image[:, :, :3]
            index += 1
        ...
        for num_frame in frams:
            image = cv.imread(
                "../DATA/PyElastica/PyElasticaRenderingImage/projection_centerline_3d_gravity/diag/frame_%04d.png"
                % int(num_frame - 30))[0:row_size, 100:col_size + 100]
            image[:, col_size - 50:] = 1 * 255
            image[1050:, :] = 1 * 255
            #pdb.set_trace()
            IMAGES[row_size * 1:row_size * 2,
                   index * col_size:(index + 1) * col_size] = image
            index += 1
        ...
        for num_frame in frams:
            image = cv.imread(
                "../DATA/PyElastica/PyElasticaRenderingImage/projection_centerline_2d/diag/frame_%04d.png"
                % int(num_frame - 30))[0:row_size, 100:col_size + 100]
            image[:, col_size - 50:] = 1 * 255
            image[1050:, :] = 1 * 255
            #pdb.set_trace()
            IMAGES[row_size * 2:row_size * 3,
                   index * col_size:(index + 1) * col_size] = image
            index += 1
        ...
        for num_frame in frams:
            image = cv.imread(
                "../DATA/PyElastica/PyElasticaRenderingImage/XPBD/projection_centerline_3d_gravity/diag/frame_%04d.png"
                % int(num_frame - 30))[0:row_size, 100:col_size + 100]
            #pdb.set_trace()
            image[:, col_size - 50:] = 1 * 255
            image[1050:, :] = 1 * 255
            IMAGES[row_size * 3:row_size * 4,
                   index * col_size:(index + 1) * col_size] = image
            index += 1
        ...
        cv.imwrite("./DATA/Connect_XPBD_Tomas.png", IMAGES)
        #pdb.set_trace()
        #plt.savefig("../DATA/ANALYSIS/Connect_XPBD_Tomas.png",dpi=200)
        #plt.show()


class MakeVideo():
    def ConnectImage(self):
        IMAGES = np.zeros((900, 1650 * 2, 3))
        for num_frame in range(30, 65):

            # ------- Raw Image -------
            image = cv.imread("../DATA/DATA_BaxterRope/rgb_raw_Baxter/%d.png" %
                              int(num_frame))[500:1400, 100:1750]
            #pdb.set_trace()
            # image[:, 1600:] = np.array([1, 1, 1]) * 255
            # image[850:, :] = np.array([1, 1, 1]) * 255
            IMAGES[:, :1650] = image[:, :, :3]
            image1 = image[:, :, :3]
            ...
            image = cv.imread(
                "../DATA/PyElastica/PyElasticaRenderingImage/projection_centerline_3d_gravity/diag/frame_%04d.png"
                % int(num_frame - 30))[0:900, 100:1750]
            #pdb.set_trace()
            # image[:, 1600:] = np.array([1, 1, 1]) * 255
            # image[850:, :] = np.array([1, 1, 1]) * 255
            IMAGES[:, 1650:] = image
            ...
            im_h = cv.hconcat([image1, image2])
            cv.imwrite("../DATA/MakeVideo/%d.png" % int(num_frame - 30), im_h)

            # plt.imshow(IMAGES)
            # plt.axis("off")

            # #plt.savefig("../DATA/MakeVideo/%d.png" % int(num_frame - 30))
            # plt.cla()

    def Video(self):

        #writer = imageio.get_writer('../DATA/MakeVideo/Baxter.mp4', fps=5)
        writer = []

        for num_frame in range(35):

            image_path = "../DATA/MakeVideo/%d.png" % int(num_frame)
            #image_path = "../DATA/DATA_BaxterRope/rgb_raw_Baxter/%d.png" % int(
            #    num_frame)
            frame = imageio.imread(image_path)
            #frame = np.resize(frame, (3312, 912))
            writer.append(frame)

        imageio.mimsave('../DATA/MakeVideo/Baxter.mp4', writer, fps=4)
...
class PlotProjectionPlane():
    def Draw(self):
        num_frame = 32
        # orignal_pcl = np.load(
        #     "/home/jingpei/ICRA2022/Experiment_data/traj/sim_2_real/3D_20pt_correspondance_length/3D_center_line/%d.npy"
        #     % num_frame)
        # # orignal_pcl = np.load("../DATA/DATA_BaxterRope/3D_center_line/%d.npy" %
        # #                       num_frame)
        # print(orignal_pcl.shape)
        # centerline_3d_gravity = np.load(
        #     "../DATA/DATA_BaxterRope/centerline_3d_gravity_clean/%d.npy" %
        #     num_frame)
        # print(centerline_3d_gravity.shape)

        # fig_curve = plt.figure()
        # ax_curve = Axes3D(fig_curve)

        # ax_curve.scatter(orignal_pcl[:, 0], orignal_pcl[:, 1], orignal_pcl[:,
        #                                                                    1])
        # ax_curve.scatter(centerline_3d_gravity[:, 0],
        #                  centerline_3d_gravity[:, 1], centerline_3d_gravity[:,
        #                                                                     1])
        #pdb.set_trace()
        #plt.style.use("seaborn")
        fig = plt.figure()
        ...
        downsampled_data = np.load(
            "../DATA/DATA_BaxterRope/3D_center_line/%d.npy" % num_frame)
        centerline = np.load(
            "../DATA/DATA_BaxterRope/centerline_3d_gravity_clean/%d.npy" %
            num_frame)
        ax.scatter(downsampled_data[:, 0],
                   downsampled_data[:, 1],
                   downsampled_data[:, 2],
                   'green',
                   label="Original PCL")
        ax.scatter(centerline[:, 0],
                   centerline[:, 1],
                   centerline[:, 2],
                   '#FF5C58',
                   label="Projection PCL")

        points = np.zeros((3, 3))
        points[0] = centerline[0]
        points[-1] = centerline[-1]
        points[1] = (centerline[0] + centerline[-1]) / 2 + np.array(
            [0.10381715, 0.9815079, -0.16082364]) * 0.20
        gravity = np.array([0.10381715, 0.9815079, -0.16082364])
        middle = (centerline[0] + centerline[-1]) / 2
        #points[1] = points[1] * 10

        max_x = downsampled_data[-1, 0]
        min_x = downsampled_data[0, 0]
        x_distance = (max_x - min_x) / 20

        max_y = downsampled_data[-1, 1]
        min_y = downsampled_data[0, 1]
        y_distance = (max_y - min_y) / 20
        ...
        point = np.array(p0)
        normal = np.array(u_cross_v) * 10

        d = -point.dot(normal)
        #pdb.set_trace()

        xx, yy = np.meshgrid(
            np.arange(min_x, max_x + x_distance * 2, x_distance),
            np.arange(min_x + x_distance * 6, max_x, x_distance))
        ...
        offset = -0.04
        arrow = Arrow3D([middle[0], points[1, 0]], [middle[1], points[1, 1]],
                        [middle[2] - offset, points[1, 2] - offset],
                        mutation_scale=40,
                        shrinkA=0.1,
                        shrinkB=0.1,
                        lw=3,
        ...
        ax.legend()
        #ax.view_init(elev=-87., azim=-87)  #DATA_PROCESS_2
        ax.view_init(elev=-160., azim=-1)  #DATA_PROCESS_1

        ax.dist = 6
        plt.savefig("./DATA/DATA_PROCESS_1.png", dpi=200, bbox_inches='tight')
        #plt.show()
...
class DescibeRod():
    ...
    def RealtoSimFigure(self):
        ...
        for num_frame in num_frames:
            ...
            sim_node = torch.as_tensor(
                np.load(
                    "../DATA/save_loss_withConstConvergency/projection/centerline_3d_gravity/frame_%d.npy"
                    % num_frame))[:, :3]
            sim_node_2D = self.ProjectionPtfrom3DNode(sim_node)

            RGB_IMAGE = plt.imread(
                "../DATA/DATA_BaxterRope/rgb_raw_Baxter/%d.png" % num_frame)

            plt.imshow(RGB_IMAGE)
            plt.scatter(sim_node_2D[:, 0], sim_node_2D[:, 1], s=10)

            plt.savefig("./DATA/Describe_%d.png" % num_frame,
                        dpi=200,
                        bbox_inches='tight')
        ...
        for num_frame in num_frames:
            #pdb.set_trace()
            plt.axis("off")
            IMAGE = plt.imread("./DATA/Describe_%d.png" % num_frame)[150:650,
                                                                     100:850]
            plt.imshow(IMAGE)
            #plt.show()
            plt.savefig("./DATA/Describe_cut_%d.png" % num_frame,
                        dpi=200,
                        bbox_inches='tight')

            #cv.imwrite("./DATA/Describe_cut_%d.png" % num_frame, IMAGE)
    ...
    def DrawPCL(self):
        ...
        for num_frame in num_frames:
            ...
            ax_curve.scatter(sim_node[:, 0],
                             sim_node[:, 1],
                             sim_node[:, 2],
                             s=60,
                             label="Simulation Rod")
            ax_curve.plot(sim_node[:, 0], sim_node[:, 1], sim_node[:, 2])
            ax_curve.scatter(downsampled_data[:, 0],
                             downsampled_data[:, 1],
                             downsampled_data[:, 2],
                             label="PCL",
                             s=10)
            ax_curve.legend(fontsize=18)

            if num_frame == 30:

                ax_curve.view_init(elev=-87., azim=-87)  #DATA_PROCESS_2
            if num_frame == 53:
                ax_curve.view_init(elev=-66., azim=-54)  #DATA_PROCESS_2
                #pdb.set_trace()

            ax_curve.dist = 10
            #pdb.set_trace()

            plt.savefig("./DATA/DrawPCL_%d.png" % num_frame,
                        dpi=150,
                        bbox_inches='tight')
            plt.cla()
    ...
    def ConnectImage(self):
        ...
        im1 = cv.imread("./DATA/DrawPCL_%d.png" % 30)
        im2 = cv.imread("./DATA/DrawPCL_%d.png" % 53)

        im_h = cv.hconcat([im1, im2])

        cv.imwrite("./DATA/ConnectImage_DrawPCL.png", im_h)

        im1 = cv.imread("./DATA/Describe_cut_%d.png" % 30)
        im2 = cv.imread("./DATA/Describe_cut_%d.png" % 53)

        im_h = cv.hconcat([im1, im2])

        cv.imwrite("./DATA/ConnectImage_Describe_cut.png", im_h)

        im1 = cv.imread("./DATA/ConnectImage_Describe_cut.png")
        im2 = cv.imread("./DATA/ConnectImage_DrawPCL.png")
        pdb.set_trace()
        #im2 = cv.resize(im2, (1300, 350))
        #im1 = cv.resize(im1, (im2.shape[1], im2.shape[0]))

        #pdb.set_trace()

        im_h = cv.vconcat([im1, im2])

        cv.imwrite("./DATA/final_describe.png", im_h)
...
class ParameterOptimization():
    def analysis(self):
        ...
        for num_frame in num_frames:
            ...
            frame_30_ori = np.load(
                "../DATA/optimal_para_data/Tomas_30_54_gravity/frame_%d_itr_0.npy"
                % num_frame)[:, 1:4]
            frame_30 = np.load(
                "../DATA/save_loss_withConstConvergency/projection/centerline_3d_gravity/frame_%d.npy"
                % num_frame)
            ...
            ax_curve.scatter(frame_30_ori[:, 0], frame_30_ori[:, 1],
                             frame_30_ori[:, 2])

            ax_curve.plot(frame_30_ori[:, 0],
                          frame_30_ori[:, 1],
                          frame_30_ori[:, 2],
                          label="Before Optimization")

            ax_curve.scatter(frame_30[:, 0], frame_30[:, 1], frame_30[:, 2])
            ax_curve.plot(frame_30[:, 0],
                          frame_30[:, 1],
                          frame_30[:, 2],
            ...
            ax_curve.scatter(downsampled_data[:, 0],
                             ...
                             downsampled_data[:, 2],
                             label="Orignal PCL")
            ax_curve.legend(loc='lower left', fontsize=18)
            plt.locator_params(axis="z", nbins=4)
            plt.locator_params(axis="x", nbins=4)
            plt.locator_params(axis="y", nbins=4)

            ax_curve.view_init(elev=-19., azim=-88)
            #plt.show()
            #pdb.set_trace()
            ax_curve.dist = 7
            plt.savefig("./DATA/ParameterOptimization_%d.png" % num_frame,
                        dpi=200,
                        bbox_inches='tight')

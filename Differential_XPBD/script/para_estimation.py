...
class PBDRope():
    def __init__(self, start_frame, end_frame, save_dir, LR_RATE, LAYERS,
        ...
        self.optimal_frame = optimal_frame
        ...
        # self.optimizer = optim.Adam([self.w_gravity],
        #                             lr=self.LR_RATE,
        #                             weight_decay=0.000)
        ...
        # self.optimizer = optim.Adam([self.w_dis],
        #                             lr=self.LR_RATE,
        #                             weight_decay=0.000)
...
# -------- Save the Optimization Data --------
    ...
    def SaveData(self, save_weight, save_loss_itr, save_curr_itr_time,
                 save_total_itr_time, save_nodes_pos_itr,
                 save_nodes_radius_itr, save_nodes_quaternion_itr, C_dis_itrs,
                 C_bending_itrs, C_strain_itrs, node_num, save_dir, num_frame,
                 itr):
        ### ==========================================================================================
        ### SAVE DATA
        ...
        #pdb.set_trace()
        np.save(save_dir + "/frame_" + str(num_frame) + "_itr_%d.npy" % itr,
                save_optimized_data)
...
class GetLoss():
    ...
    def getProjectionLoss(self, cal_loss_target, node_pos_curr, num_frame,
        ...
        if cal_loss_target == 'centerline_2d_plus_lwst':
            ...
            loss = loss_projection_lineseg_2D + loss_PointAlongGraivity * 100
...
class ExecuteBackward():
    def Execute(self, start_frame, end_frame, save_dir, LR_RATE, LAYERS, STEPS,
                ...
                cal_loss_target, optimal_frame, use_XPBD):
        ## ================================================================================
        ...
        for itr in range(BaxterRope.ITERATION):
            ...
            for num_frame in BaxterRope.optimal_frame:
                    ...
                    for i in range(BaxterRope.LAYERS):
                        ...
                        if use_XPBD == False:
                            ...
                            dx_dist, Lambda_dist, C_dis = Diff_dist.solveDistConstraint(
                                node_pos_curr, Lambda_dist, BaxterRope.wab,
                                BaxterRope.wab)
                            dx_dist[0] = dx_dist[0] * 0
                            dx_dist[-1] = dx_dist[-1] * 0
                        #pdb.set_trace()
                ...
                #     node_pos_curr, centerline_3d_gravity)
                DiffLoss = GetLoss()
                loss_3D = DiffLoss.CalculateLoss(cal_loss_method,
                                                 cal_loss_target,
                                                 node_pos_curr, num_frame,
                                                 BaxterRope.cam_intrinsic_mat)
                ...
                # -------- Save optimal result --------
                save_weight = np.zeros((BaxterRope.node_num, 1))
                #pdb.set_trace()
                save_weight[:6] = np.array([
                    BaxterRope.w_gravity, BaxterRope.w_dis,
                    BaxterRope.w_strain, BaxterRope.wq_strain,
                    BaxterRope.wq_bending, BaxterRope.w_SOR
                ]).reshape(-1, 1)
                save_weight[6] = loss_3D.detach().numpy()
                ...
                OptimizedData.SaveData(
                    save_weight, save_loss_itr, save_curr_itr_time,
                    save_total_itr_time, save_nodes_pos_itr,
                    save_nodes_radius_itr, save_nodes_quaternion_itr,
                    C_dis_itrs, C_bending_itrs, C_strain_itrs,
                    BaxterRope.node_num, BaxterRope.save_dir, num_frame, itr)
                ### ==========================================================================================
                #### -------- Print loss for Frame--------
                print("No. Frame :", num_frame)
                print("The loss  :", loss_3D)
            #pdb.set_trace()
            ### ==========================================================================================

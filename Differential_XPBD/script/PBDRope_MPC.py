...
class PBDRope():
    def __init__(self, start_frame, end_frame, save_dir, LR_RATE, LAYERS,
        ...
        self.update_pos_actuated = torch.zeros((self.MPC_STEPS, 3))
        self.update_pos_actuated[:] = self.node_pos_curr[0].clone()
...
class ProcessOptData():
    ...
    # -------- Save temp data --------
    def SaveTempData(self, node_pos_curr, loss_temp, C_dis, C_strain,
                     C_bending, itr, j):

        Save_Temp_Data = np.hstack(
            (node_pos_curr, np.insert(C_dis, 19, loss_temp).reshape(-1, 1),
             np.insert(np.linalg.norm(C_strain, axis=1), 19, 0).reshape(-1, 1),
             np.insert(np.linalg.norm(C_bending, axis=1), 18,
                       [0, 0]).reshape(-1, 1)))
        #pdb.set_trace()

        np.save(
            "../DATA/CompareOverXPBDAndTomas/Tomas_54/" + str(itr) + "_" +
            str(j) + ".npy", Save_Temp_Data)
    ...
    def Execute(self, start_frame, end_frame, save_dir, LR_RATE, LAYERS, STEPS,
        ...
        for num_frame in range(BaxterRope.start_frame, BaxterRope.end_frame):
                ...
                BaxterRope.node_pos_curr = torch.as_tensor(
                    np.load(BaxterRope.save_dir +
                            "/frame_{}.npy".format(num_frame -
                                                   1))[:, :3]).float()
            diff = torch.diff(BaxterRope.node_pos_curr, dim=0)
            diff = torch.sum(
                torch.linalg.norm(torch.diff(BaxterRope.node_pos_curr, dim=0),
                                  dim=1))

            #diff = torch.sum(torch.linalg.norm(torch.diff(node_pos_curr, dim=0), dim=1))
            #pdb.set_trace()
            ...
            for itr in range(BaxterRope.ITERATION):
                ...
                # -------- Save the MPC steps simulation node information --------
                mpc_sim_nodes = torch.zeros(
                    (BaxterRope.MPC_STEPS, BaxterRope.node_num, 3))
                for mpc_step in range(BaxterRope.MPC_STEPS):
                    #pdb.set_trace()
                    loss = 0
                    ...
                    for j in range(BaxterRope.STEPS):
                        ...
                        for i in range(BaxterRope.LAYERS):
                            ...
                            dx_strain, Lambda_strain, dq_strain, C_strain = Diff_stretch.C_strain_solve(
                                node_pos_curr, Lambda_strain,
                                node_quaternion_curr, BaxterRope.ws,
                                BaxterRope.ws, BaxterRope.wq)
                            ...
                            # node_pos_curr = node_pos_curr + dx_strain * w_strain
                            node_pos_curr = node_pos_curr + (
                                dx_strain * BaxterRope.w_strain + dx_dist *
                                BaxterRope.w_dis) / 2 * BaxterRope.w_SOR
                        ...
                        # diff = torch.sum(
                        #     torch.linalg.norm(torch.diff(node_pos_curr, dim=0),
                        #                       dim=1))
                        # #print(diff)

                        # OptimizedData.SaveTempData(node_pos_curr, diff,
                        #                            C_dis.detach().numpy(),
                        #                            C_strain.detach().numpy(),
                        #                            C_bending.detach().numpy(), itr,
                        #                            j)
                    ...
                    #     node_pos_curr, centerline_3d_gravity)
                    DiffLoss = GetLoss()
                    loss = loss + DiffLoss.CalculateLoss(
                        cal_loss_method, cal_loss_target, node_pos_curr,
                        num_frame, BaxterRope.cam_intrinsic_mat)
                    mpc_sim_nodes[mpc_step] = node_pos_curr
                    #pdb.set_trace()
                ...
                BaxterRope.node_pos_curr = mpc_sim_nodes[0]
